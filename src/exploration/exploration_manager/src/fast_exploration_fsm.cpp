...
void FastExplorationFSM::visualize() {
  ...
  for (int i = 0; i < ed_ptr->frontiers_.size(); ++i) {
    visualization_->drawCubes(ed_ptr->frontiers_[i], 0.1,
                              visualization_->getColor(double(i) / ed_ptr->frontiers_.size(), 1),
                              "frontier", i, 4);
  ...
  // visualization_->drawLines(ed_ptr->refined_tour_, 0.05, Vector4d(0, 0, 1, 1), "refined_tour", 0, 6);
  ...
  if (state_ == WAIT_TRIGGER || state_ == FINISH) {
    ...
    for (int i = 0; i < ed->frontiers_.size(); ++i) {
      visualization_->drawCubes(ed->frontiers_[i], 0.1,
                                visualization_->getColor(double(i) / ed->frontiers_.size(), 1),
                                "frontier", i, 4);
